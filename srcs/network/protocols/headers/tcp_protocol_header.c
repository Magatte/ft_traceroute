...
# ifdef __linux__
	...
	message->tcp_header.checksum = tcp_checksum(trace->protocol->len, message->ip_header.ip_src.s_addr,\
		message->ip_header.ip_dst.s_addr, message->tcp_header)//checksum(trace->protocol->len, trace->protocol->len + size);
	ft_memcpy(ptr_message + iphdr_size, &message->tcp_header, trace->protocol->len);
# endif
}
